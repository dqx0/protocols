...
func (c *Client) Do(r *Request) (*Response, error) {
    if r == nil { return nil, errors.New("httpx: nil request") }
    if c.Transport == nil {
        bt := NewBasicTransport()
        if c.MaxConnsPerHost > 0 { bt.MaxConnsPerHost = c.MaxConnsPerHost }
        if c.IdleConnTimeout > 0 { bt.IdleConnTimeout = c.IdleConnTimeout }
        c.Transport = bt
    }
    max := c.MaxRedirects
    if max <= 0 { max = 10 }
    ...
    for i := 0; ; i++ {
        ...
        _ = res.Body.Close()
        if i+1 > max { return nil, errors.New("httpx: too many redirects") }
...
func (c *Client) Post(rawurl, ctype string, body io.Reader) (*Response, error) {
    ...
    if body != nil {
        if _, err := io.Copy(&buf, body); err != nil { return nil, err }
    }
...
func hostOnly(h string) string {
    if i := strings.LastIndex(h, ":"); i != -1 {
        if strings.HasPrefix(h, "[") && strings.HasSuffix(h, "]") { return strings.Trim(h, "[]") }
        return h[:i]
    }
    return h
}
