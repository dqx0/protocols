...
func TestReader_ContentLengthBody(t *testing.T) {
    ...
    if err != nil { t.Fatalf("ReadRequest error: %v", err) }
    if pr.ContentLength != 5 { t.Fatalf("ContentLength=%d", pr.ContentLength) }
    b, _ := io.ReadAll(pr.Body)
    if string(b) != "hello" { t.Fatalf("body=%q", string(b)) }
}
...
func TestReader_ChunkedBody(t *testing.T) {
    ...
    pr, err := readReq(t, raw, 8<<10, 64<<10)
    if err != nil { t.Fatalf("ReadRequest error: %v", err) }
    if pr.ContentLength != -1 { t.Fatalf("ContentLength=%d", pr.ContentLength) }
    b, _ := io.ReadAll(pr.Body)
    if string(b) != "hey!!" { t.Fatalf("body=%q", string(b)) }
}
